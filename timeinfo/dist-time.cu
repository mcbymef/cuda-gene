...
int main(int argc, char** argv) {
    ...
    if(calculateAllGenes && canUseCuda) {
        ...
        printf("Elapsed serial time: %f ms\n", elapsedTime); 
        exit(0);
...
void getCardSpecs() {
    ...
    if(props.totalGlobalMem == 0) {
        printf("Warning: No CUDA card detected. Program will contine in serial mode.\n");
        canUseCuda = 0;
    } else if(props.totalGlobalMem < geneListSize + resultsSize) {
        printf("Warning: CUDA card has insufficient total memory to run for this input size. Program will continue in serial mode.\n");
